...
- Run `mise exec -- pnpm dlx @tanstack/intent@latest list` from the `frontend/` directory to see available local skills.
- If a listed skill matches the task, run `mise exec -- pnpm dlx @tanstack/intent@latest load <package>#<skill>` from the `frontend/` directory before changing files.
